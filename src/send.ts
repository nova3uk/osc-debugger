...
async function sendMessage(
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.send(message, 0, message.length, port, address, (err?: Error | null) => {
      if (err) {
        reject(err);
      } else {
        resolve();
...
export default async function send({ port, address = null, version = 'unknown' }: SendOptions): Promise<void> {
  try {
    // Validate port
    if (port < 1 || port > 65535) {
      ...
      const input: Answers = await inquirer.prompt([
        {
          ...
          validate: (value: string) => {
            ...
            if (!ipRegex.test(value) && value !== '0.0.0.0' && value !== 'localhost') {
    ...
    console.log(`  - Value: Number or string in quotes (e.g., 42 or "red")`.gray);
    console.log(`\nExample:`.gray);
    console.log(`  /light/1/color "red"`.cyan);
    console.log(`  /volume 0.75`.cyan);
    console.log(`  /status 1`.cyan);
    console.log(`\nType your OSC messages below (Ctrl+C to exit):\n`.gray);
    ...
    while (true) {
      try {
        ...
        try {
          ...
          console.error(
            `Error parsing value: ${err instanceof Error ? err.message : String(err)}`.red
          );
          continue;
        }
        ...
        const message: Buffer = Buffer.from(messageBuffer.buffer, messageBuffer.byteOffset, messageBuffer.byteLength);
        ...
        console.error(
          `Error sending message: ${err instanceof Error ? err.message : String(err)}`.red
        );
      }
    }
  } catch (err) {
